...
def open_win(): #OPENS MAIN MENU----------------------------------------------------------------------------MAIN MENU
    ...
    Button(apt,text='Add product to Stock', width=25,command=stock).grid(row=4,column=0)
    Button(apt,text='Delete product from Stock', width=25,command=delete_stock).grid(row=5,column=0)
...
def make_bill(): # makes bill
    ...
    m+="  Indira College of Commerce and Science \n\n"
    m+="-----------------------------------------------\n"
    if t==1:
        m+="Name: %s\n" % named
        m+="Address: %s\n" % addd
...
def again():    #for login window-----------------------------------------------------------------------------LOGIN WINDOW
    ...
    Label(root,text=" Indira College of Commerce and Science ").grid(row=1,column=0,columnspan=5)
    ...
    Button(root,width=10,text='Login',command=check).grid(row=5, column=0)
    Button(root,width=10,text='Close',command=root.destroy).grid(row=5, column=1)
    root.mainloop()
...
def buy_now():
    ...
    buy=Tk()
    buy.title('BUY NOW')
    Label(buy, text=' SELECT YOUR MEDICINE').grid(row=0, column=0,columnspan=3)
    Label(buy, text='~'*40).grid(row=1, column=0,columnspan=3)
    Label(buy, text='ALL MEDICINES').grid(row=3, column=0)
    sym=Spinbox(buy,values=med_name)
    sym.grid(row=3, column=1)
    Button(buy,width=15, text='BUY NOW', command=lambda: func("BOUGHT SUCCESFULLY")).grid(row=3, column=2)
    Label(buy, text='-'*70).grid(row=4, column=0,columnspan=3)
    if flags=='apt1':
        Button(buy,width=15, text='Main Menu', command=main_cus).grid(row=6, column=2)
    else:
        Button(buy,width=15, text='Main Menu', command=main_menu).grid(row=6, column=2)
    buy.mainloop()
...
def open_cus(): #OPENS MAIN MENU----------------------------------------------------------------------------MAIN MENU
    ...
    Button(apt,text='Buy Now', width=15, command=buy_now).grid(row=7,column=0)    
